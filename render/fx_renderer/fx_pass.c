...
static void set_proj_matrix(GLint loc, float proj[9], const struct wlr_box *box) {
	float gl_matrix[9];
	matrix_identity(gl_matrix);
	matrix_translate(gl_matrix, box->x, box->y);
	matrix_scale(gl_matrix, box->width, box->height);
	matrix_multiply(gl_matrix, proj, gl_matrix);
...
static void set_tex_matrix(GLint loc, enum wl_output_transform trans,
	...
	float tex_matrix[9];
	matrix_identity(tex_matrix);
	matrix_translate(tex_matrix, box->x, box->y);
	matrix_scale(tex_matrix, box->width, box->height);
	matrix_translate(tex_matrix, .5, .5);
